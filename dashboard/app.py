...
with st.sidebar:
    ...
    # --- FIX: CLEAR FILTERS MUST RUN BEFORE WIDGETS ---
    if st.button("🔄 Clear Filters", key="clear_filters_btn"):
        st.session_state["ns_filter"] = ""
        st.session_state["pod_filter"] = ""
        st.session_state["reason_filter"] = ""
        st.session_state["search_text"] = ""
        st.session_state["sort_by"] = "start_ts"
        st.session_state["order"] = "desc"
        st.session_state["limit"] = 100
        st.rerun()

    # --- NOW render widgets safely ---
    with st.expander("Basic Filters", expanded=True):
        ns_filter = st.text_input("Namespace", value=st.session_state.get("ns_filter", ""), key="ns_filter")
        pod_filter = st.text_input("Pod Name", value=st.session_state.get("pod_filter", ""), key="pod_filter")
        reason_filter = st.text_input("Reason", value=st.session_state.get("reason_filter", ""), key="reason_filter")

    with st.expander("Advanced Query"):
        search_text = st.text_input("Vector Search", value=st.session_state.get("search_text", ""), key="search_text")

    with st.expander("Sorting & Pagination"):
        sort_by = st.selectbox("Sort By", ["start_ts", "namespace", "pod", "severity_hint", "id"], key="sort_by")
        order = st.radio("Order", ["desc", "asc"], key="order", horizontal=True)
        limit = st.number_input("Page size", min_value=10, max_value=500, value=st.session_state.get("limit", 100), key="limit")
...
for it in items:
    ...
    rows.append({
        ...
        "pod": meta.get("pod", ""),
        "node": meta.get("node", ""),
        "reason": meta.get("reason") or extract_reason(doc) or "",
