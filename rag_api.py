...
def list_logs(
    ...
    if q_f and embed_model:
        ...
        for cid, doc, meta in zip(ids, docs, metas):
            ...
            # Normalize metadata
            m_ns = norm(meta.get("namespace"))
            m_pod = norm(meta.get("pod"))
            m_reason = norm(meta.get("reason"))

            # Extract REASON from message text if missing
            if not m_reason:
                match = re.search(r"REASON=([A-Za-z0-9_-]+)", doc or "")
                if match:
                    m_reason = match.group(1).lower()
                    meta["reason"] = match.group(1)

            # Apply filters
            if ns_f and m_ns != ns_f:
                continue
            if pod_f and m_pod != pod_f:
                continue
            if reason_f and m_reason != reason_f:
                continue
